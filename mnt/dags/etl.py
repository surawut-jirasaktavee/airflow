...
def _fetch_ohlcv(**context):
    ...
    ohlcv = exchange.fetch_ohlcv("ETH/USDT", timeframe="1h", since=millisec, limit=24)
    logging.info(f"OHLCV of EHT/USDT value. [ohlcv={ohlcv}]")

    # Write OHLCV data to CSV file
    with open(f"eth-{ds}.csv", "w") as f:
        writer = csv.writer(f)
    ...
    s3_hook.load_file(
        f"eth-{ds}.csv",
        key=f"cryptocurrency/{ds}/eth.csv",
